...
export function parseDeckMarkdown(markdown: string, fallbackTitle = "Présentation"): Presentation {
  ...
  slides.push({
    ...
    category: (fmBrand ?? "Nonvi-Efa").toUpperCase(),
  });
  ...
  if (!hasThanks && slides.length > 3) {
    slides.push({
      kind: "thanks",
      title: "merci",
      body: "Cette présentation a été générée automatiquement par la plateforme Nonvi-Efa.",
      resources: ["Nonvi-Efaagency.com"],
    });
  }

  return {
    title: deckTitle,
    brand: fmBrand ?? "Nonvi-Efa",
    date: fmDate,
    slides,
  };
}
