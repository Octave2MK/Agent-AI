...
export async function POST(req: Request) {
  if (!apifyConfigured()) {
    return Response.json(
      {
        error:
          "Apify n'est pas configuré : ajoutez APIFY_TOKEN=... dans Nonvi-Efa-platform/.env.local (token gratuit sur console.apify.com → Settings → API & Integrations), puis relancez le serveur.",
        needsConfig: true,
      },
      { status: 412 }
    );
