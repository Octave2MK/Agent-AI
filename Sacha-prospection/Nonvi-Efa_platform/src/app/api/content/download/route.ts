...
function slideHTML(t: { bg: string; fg: string; accent: string; sub: string; font: string }, title: string, body: string, i: number, total: number) {
  ...
  return `<div class="slide" style="background:${t.bg};color:${t.fg};font-family:${t.font}">
    <div class="row"><span class="brand" style="color:${t.accent}">Nonvi-Efa</span>${!cover ? `<span class="pg" style="color:${t.sub}">${i + 1}/${total}</span>` : ""}</div>
    <div class="mid">
    ...
    <div class="row foot" style="color:${t.sub}"><span>@Nonvi-Efa.agency</span><span style="color:${t.accent}">${cover ? "Swipe →" : last ? "↗ Contactez-nous" : "→"}</span></div>
  </div>`;
}
    ...
    try {
      ...
      return new Response(new Uint8Array(pdf), {
        headers: {
          ...
          "Content-Disposition": `attachment; filename="Nonvi-Efa-${post.platform}-${post.id}.pdf"`,
        },
      });
