...
function instructions(platform: Platform, format: Format, template?: string): string {
  ...
  if (platform === "linkedin" && format === "image") return `Format : POST LinkedIn IMAGE + texte. "headline" = accroche forte qui ira EN GROS sur le visuel (≤ 12 mots). "body" = le post LinkedIn dans la VOIX de Zeyneb.\n${LI_VOICE}\nRéponds en JSON: {"headline":"","body":"","hashtags":["#..."]}.${tmpl}`;
  if (platform === "linkedin") return `Format : POST LinkedIn texte, dans la VOIX de Zeyneb.\n${LI_VOICE}\nRéponds en JSON: {"body":"le post complet avec sauts de ligne \\n","hashtags":["#..."]}.`;
